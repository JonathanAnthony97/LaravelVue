	...
	<body >
		<div>
			...
			<div style="margin-top: -10px;text-align: center;">
				<p >
					<b>BORDEREAU DESCRIPTIF DE<br> REPARTITION
			...
			<div>
					<table style="width: 100%;">
						...
						<tr>
							...
							<td>{{ $item['design'] }}</td>
							<td>{{ $item['source'] }}</td>
							<td>{{ number_format($item['sortie'],0,'.',' ')  }}</td>
					...
					ARRETEE LE PRESENT BORDEREAU AU NOMBRE DE ({{ $counter }}) ARTICLE{{ $counter > 0 ? 'S' : '' }} ET AU TOTAL DE ({{number_format($total,0,'.',' ') }})
