	...
	<body >
		<div>
			...
			<div style="margin-top: -10px;text-align: center;">
				<p >
					<b>BORDEREAU DESCRIPTIF DE<br> L'ETAT DE STOCK
			...
			<div>
					<table style="width: 100%;">
						...
						<tr>
							...
							<td>{{ $item->design }}</td>
							<td>{{ $item->source }}</td>
							<td>{{ number_format($item->qte_recue,0,'.',' ') }}</td>
							<td>{{ number_format($item->qte_distribue,0,'.',' ') }}</td>
							<td>{{ number_format($item->qte_stock,0,'.',' ') }}</td>
						</tr>
						@endforeach
						<tr>
							<th colspan="3" style="text-align: right;">TOTAL</th>
							<td>{{ number_format($total_recue,0,'.',' ') }}</td>
							<td>{{ number_format($total_distribue,0,'.',' ') }}</td>
							<td>{{ number_format($total_stock,0,'.',' ') }}</td>
						</tr>
					</table>
					<p style="margin-top: 10px;">
					ARRETEE LE PRESENT BORDEREAU AU NOMBRE DE ({{ $counter }}) ARTICLE{{ $counter > 0 ? 'S' : '' }} ET AU STOCK TOTAL DE ({{number_format($total_stock,0,'.',' ')}})
